...
CREATE TABLE IF NOT EXISTS adaptive_threshold_history (
    ...
    INDEX idx_threshold_history_context (context_type, context_value),
    INDEX idx_threshold_history_time (created_at),
    INDEX idx_threshold_history_type (threshold_type)
);
...
CREATE TABLE IF NOT EXISTS user_behavior_patterns (
    ...
    INDEX idx_user_behavior_pattern (content_pattern_hash),
    INDEX idx_user_behavior_updated (last_updated),
    INDEX idx_user_behavior_confidence (learning_confidence)
);
...
CREATE TABLE IF NOT EXISTS learning_parameters (
    ...
    -- Indexes
    INDEX idx_learning_params_name (parameter_name),
    INDEX idx_learning_params_scope (context_scope, scope_identifier)
);
...
CREATE TABLE IF NOT EXISTS threshold_contexts (
    ...
    -- Indexes
    INDEX idx_threshold_contexts_type (context_type),
    INDEX idx_threshold_contexts_learning (learning_enabled),
    INDEX idx_threshold_contexts_updated (updated_at)
);
...
CREATE TABLE IF NOT EXISTS learning_feedback_signals (
    ...
    -- Indexes
    INDEX idx_feedback_signals_type (signal_type),
    INDEX idx_feedback_signals_processed (processed),
    INDEX idx_feedback_signals_time (created_at),
    INDEX idx_feedback_signals_context (context_id)
);
...
('time_period', 'off_hours', 'Low activity periods'),
