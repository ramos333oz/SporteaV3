// Create a high-similarity test match for comprehensive testing
// This will generate a characteristic vector that closely matches Azmil's preference vector

// Azmil's preference vector (from database)
...
// Generate a high-similarity vector (85% similar to Azmil's preferences)
function generateHighSimilarityVector(baseVector, similarityTarget = 0.85) {
  const highSimilarityVector = [];
  
  for (let i = 0; i < baseVector.length; i++) {
    // Create high similarity by using mostly the base vector with small random variations
    const randomVariation = (Math.random() - 0.5) * 0.3; // Small random variation
...
// Generate the high-similarity vector
const highSimilarityVector = generateHighSimilarityVector(azmilPreferenceVector, 0.85);
...
console.log('High-Similarity Test Match Vector Generated!');
...
// Generate SQL statements for creating the test match
const vectorString = '[' + highSimilarityVector.join(',') + ']';

console.log('\n=== SQL STATEMENTS FOR HIGH-SIMILARITY TEST ===');
console.log('\n1. Create the high-similarity test match:');
console.log(`
INSERT INTO matches (
  id, title, description, sport_id, skill_level, start_time, end_time, 
  max_participants, location_id, host_id, status, created_at, updated_at
) VALUES (
  gen_random_uuid(),
  'Perfect Basketball Match - High Similarity Test',
  'This basketball match is specifically designed to have high similarity with Azmil''s preferences for comprehensive testing of the recommendation system. Beginner-friendly casual basketball session.',
  'dd400853-7ce6-47bc-aee6-2ee241530f79',
  'Beginner',
  '2025-07-06 09:00:00+00',
  '2025-07-06 10:00:00+00',
  8,
  '442107e5-4100-4f61-832f-6a95d52f4ac5',
  'a7ed4757-5983-4112-967f-b678430248f9',
  'active',
  NOW(),
  NOW()
) RETURNING id;
...
console.log('\n2. Update with high-similarity characteristic vector:');
console.log(`UPDATE matches SET characteristic_vector = '${vectorString}' WHERE title = 'Perfect Basketball Match - High Similarity Test';`);

console.log('\n3. Verify the match was created:');
console.log(`SELECT id, title, characteristic_vector IS NOT NULL as has_vector FROM matches WHERE title = 'Perfect Basketball Match - High Similarity Test';`);
